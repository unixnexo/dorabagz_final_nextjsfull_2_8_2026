...
export function BottomNav({
    ...
    return (
        <nav
            className="fixed inset-x-0 bottom-0 z-30 mx-auto w-full max-w-[500px] px-4 pb-[calc(20px+env(safe-area-inset-bottom))]"
        >
            <div className="flex h-[74px] items-center justify-around rounded-[28px] bg-brand-primary shadow-[0_8px_30px_rgba(0,0,0,0.18)]">
                {navItems.map((item) => {
                        ...
                        <>
                            {isActive && (
                                <motion.div
                                    ...
                                    className="absolute h-[64px] w-[90%] rounded-[22px] bg-brand-secondary"
                                    transition={{
                                        type: "spring",
                                        stiffness: 500,
                                        damping: 32,
