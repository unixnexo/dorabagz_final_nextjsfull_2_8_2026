...
export type ProductListItemDTO = {
  ...
  hasDiscount: boolean; // true if ANY variant has an active discount
  totalStock: number; // sum of all variant stock
  // Favorite state for the CURRENT viewer — always false for logged-out
  // visitors (there's no user to check against).
  isFavorited: boolean;
  // Review summary, counting only APPROVED reviews (same visibility rule
  // as the product detail page's review list — pending/rejected reviews
  // never affect this). averageRating is the rounded whole-star value
  // (e.g. 3 users rating 4,4,5 -> averageRating: 4), and is 0 when
  // reviewCount is 0 (no reviews yet, not "0 stars").
  reviewCount: number;
